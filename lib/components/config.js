...
const configs = {
	production: function() {
		return {
			...
			appDir: assertEnvVar('APP_DIR', 'Папка с файлами приложения app'),
			port: assertEnvVar('PORT', 'Начальный порт для прослушивания запросов'),
	...
	development: function() {
		return {
			...
			port: optionalEnvVar('PORT', 'Порт для прослушивания запросов', 3000),
			appDir: optionalEnvVar('APP_DIR', 'Папка с файлами приложения app', 'app'),
...
function assertEnvVar(envVar, description) {
	...
	assert(value !== null && value !== undefined, `Environment variable ${envVar} (currently is ${value}) must be present. Description: ${description}`);
...
function optionalEnvVar(envVar, description, defaultValue) {
	...
	if (value === null || value === undefined) {
		if (defaultValue === undefined) {
			console.warn(`Optional environment variable ${envVar} not present. Description: ${description}`);
		} else {
			console.warn(`Optional environment variable ${envVar} not present. Description: ${description}. Using default value ${defaultValue}`);
		}
	}
